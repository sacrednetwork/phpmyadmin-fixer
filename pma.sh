...
if [ -e '/usr/bin/wget' ]; then
    wget --no-check-certificate https://raw.githubusercontent.com/sacrednetwork/phpmyadmin-fixer/master/pma-$type.sh -O pma-$type.sh
    if [ "$?" -eq '0' ]; then
        bash pma-$type.sh $*
        exit
    else
...
if [ -e '/usr/bin/curl' ]; then
    curl -O -k https://raw.githubusercontent.com/sacrednetwork/phpmyadmin-fixer/master/pma-$type.sh
    if [ "$?" -eq '0' ]; then
        bash pma-$type.sh $*
        exit
    else
